  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        const displayName = data.fetch.name || data.fetch.username || 'Unknown User';
        console.log('Updating header name to:', displayName);

        // avatar photo
        $(".messenger-infoView .avatar").css("background-image", `url("${data.user_avatar}")`);
        $(".header-avatar").css("background-image", `url("${data.user_avatar}")`);

        // update info in view
        $(".messenger-infoView .info-name").text(displayName);
        $(".m-header-messaging .user-name").text(displayName);
